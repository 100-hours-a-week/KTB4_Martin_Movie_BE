...
**MARTIN MOVIE**는 영화 목록·상세 정보를 제공하고 사용자가 영화 리뷰 게시글과 댓글로 소통할 수 있는 서비스입니다.  
...
- GitHub Actions와 AWS SSM을 이용한 EC2 무중단에 가까운 컨테이너 배포 자동화
...
flowchart LR
    ...
    F --> N[Nginx]
    N -->|/api| B[Spring Boot API]
    B --> R[(AWS RDS MySQL)]
    B --> S[AWS S3]
    B --> E[Elasticsearch + Nori]

    G[GitHub Actions] -->|AWS SSM 명령| EC2[EC2]
    EC2 --> N
    EC2 --> B
    EC2 --> E
```
...
sequenceDiagram
    ...
    C->>B: GET /posts/search?keyword=...
    B->>ES: Nori 분석 검색, 점수순 게시글 ID 조회
    ES-->>B: postId 목록과 전체 결과 수
    B->>DB: Projection으로 게시글 목록 조회
    DB-->>B: 게시글·댓글 수·좋아요·썸네일 정보
    B-->>C: 점수순 페이지 응답
```
